...
export default async function K1S654(
  STA: EnumSTA
): Promise<number> {
  ...
  switch (STA) {
    ...
    case EnumSTA.BS_EN_1992_3_NA:
      throwCalculatorError("{{{Does not apply in the UK - see PD 6687 for an alternative Annex J}}}", PropertyCode.K1S654, undefined, NaN);
      return NaN;
  }
}