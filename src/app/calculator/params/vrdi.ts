import { EnumCON, EnumSREP, ErrorLevel, PropertyCode } from '../common/enums';
...
export default async function VRDI(
  ...
  let fctd = (await getSettingValue(CON, PropertyCode.FCTK005)) / GACF;
  ...
  let vrdi: number = Math.min(
    cu * fctd + mu * sign + ro * fyd * (mu * Math.sin(alpha) + Math.cos(alpha)),
    0.5 * v * fcd
  );
...
export async function checkVRDI(
  ...
  let fctd: number =
    (await Common.getSettingValue(CON, PropertyCode.FCTK005)) / GACF;
  let ac: number = BFL * HFL;
  4;
  let fyd: number = FYK / GSF;
  let v = 0.6 * (1 - FCK / 250);
  ...
  if (VEDI < VRDI) {
    ...
    if (0.5 * v * fcd > VEDI) {
      let Asw_smin =
        ((ai * (VEDI - cu * fctd - mu * sign)) /
          (fyd * (mu * Math.sin(alpha) + Math.cos(alpha)))) *
        10000;
      throwCalculatorError(
        `Vedi > Vrdi, contrainte de cisaillement non admissible; section d'acier nécessaire pour respecter la contrainte: ${Asw_smin} cm²/ml`,
        'VRDI',
        undefined,
        VRDI,
        ErrorLevel.ERROR
      ...
      throwCalculatorError(
        'Vedi > Vrdi, contrainte de cisaillement non admissible',
        'VRDI',
        undefined,
        VRDI,
        ErrorLevel.ERROR
